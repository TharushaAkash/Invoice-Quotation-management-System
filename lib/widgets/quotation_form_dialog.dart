...
class _QuotationFormDialogState extends State<QuotationFormDialog> {
  ...
  void initState() {
    ...
    _quotationNumberController = TextEditingController(text: quotation?.quotationNumber ?? '');
    _customerNameController = TextEditingController(text: quotation?.customerName ?? '');
    _customerAddressController = TextEditingController(text: quotation?.customerAddress ?? '');
    _customerPhoneController = TextEditingController(text: quotation?.customerPhone ?? '');
    _taxRateController = TextEditingController(text: (quotation?.taxRate ?? 0.0).toString());
    _notesController = TextEditingController(text: quotation?.notes ?? '');
    _selectedDate = quotation?.date ?? DateTime.now();
    _validUntil = quotation?.validUntil ?? DateTime.now().add(const Duration(days: 30));
    _status = quotation?.status ?? 'DRAFT';

    // Initialize items
    if (quotation != null && quotation.items.isNotEmpty) {
      _items.addAll(quotation.items);
  ...
  Future<void> _generateQuotationNumber() async {
    try {
      final number = await widget.quotationService.generateQuotationNumber();
  ...
  Future<void> _selectDate(BuildContext context, bool isValidUntil) async {
    final picked = await showDatePicker(
      context: context,
      initialDate: isValidUntil ? _validUntil : _selectedDate,
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (widget.quotation?.id != null) {
        await widget.quotationService.update(quotation);
      } else {
        await widget.quotationService.create(quotation);
      }
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildSectionTitle(context, 'Quotation Info'),
                      Card(
                        elevation: 0,
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                TextFormField(
                                  ...
                                  decoration: const InputDecoration(
                                    hintText: 'Add valid period instructions or additional notes here...',
                                    border: OutlineInputBorder(),
                                  ),
                                  maxLines: 4,
                                ),
                            ...
                            child: Container(
                              ...
                              child: Column(
                                children: [
                                  ...
                                  _buildTotalRow(
                                    'Total', 
                                    _calculateSubtotal * (1 + (double.tryParse(_taxRateController.text) ?? 0) / 100),
                                    isBold: true,
                                    color: AppTheme.primaryColor,
                                    fontSize: 20,
                                  ),
              ...
              Row(
                ...
                children: [
                  ...
                  ElevatedButton.icon(
                    ...
                    label: const Text('Save Quotation', style: TextStyle(fontSize: 16)),
                  ),
                ],
              ),
            ],
